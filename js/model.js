...
M.getEventsByGroup = function(annee, group) {
    if ( annee in Events ) {
        return Events[annee].getEventsByGroup(group).map( event => event.toObject() );
    }
    return null;
}
...
M.init = async function() {
    ...
    data = await fetch('./data/mmi2.ics');
    data = await data.text();
    data = ical.parseICS(data);
    Events.mmi2 = new EventManager('mmi2', 'MMI 2', 'Agenda des MMI 2');
    Events.mmi2.addEvents(data);
    

    data = await fetch('./data/mmi3.ics');
    data = await data.text();
    data = ical.parseICS(data);
    Events.mmi3 = new EventManager('mmi3', 'MMI 3', 'Agenda des MMI 3');
    Events.mmi3.addEvents(data);
